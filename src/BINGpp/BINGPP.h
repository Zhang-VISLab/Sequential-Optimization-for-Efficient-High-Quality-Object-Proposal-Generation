...
public:
	...
	BINGPP(DataSetVOC &voc, double base = 2, int W = 8, int NSS = 2);
	BINGPP(double base = 2, int W = 8, int NSS = 2);
	~BINGPP(void);
	...
	void trainBINGPP(int numDetPerSize = 100);
...
public: // For illustration etc.
	...
	void setTheta(int i, float v);
	int getNumTheta();
	void setNumTheta(int v);
private: // Parameters
	float *thetavalue;
	int numTheta;
...
private: // Help functions
	...
	void getBINGBoxes(Mat img3u, int numDetPerSize,ValStructVec<float, Vec4i> &valBoxesTemp);
	void SegRefine(Mat img3u,ValStructVec<float, Vec4i> &valBoxes, ValStructVec<float, Vec4i> &valBoxesTemp);
	//edge based refinement (i.e. E-BING)

	void EdgeRecursiveBox(int rat, Mat img3u, Mat v, Mat u, ValStructVec<float, Vec4i> &valBoxesTemp);
	void DistanceTransform(Mat img3u, Mat edge, Mat &v, Mat &u);
	void CannyEdgeDetection(Mat img3u, Mat &edge);
	void EdgesRefine(Mat &img3u,ValStructVec<float, Vec4i> &valBoxesTemp);
