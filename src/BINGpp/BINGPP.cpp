...
const char* BINGPP::_clrName[3] = {"MAXBGR", "HSV", "I"};
...
BINGPP::BINGPP(DataSetVOC &voc, double base, int W, int NSS)
	: _voc(voc)
	, _base(base)
	, _W(W)
	, _NSS(NSS)
...
void BINGPP::initTheta()
{
	thetavalue = new float[numTheta];
}

float BINGPP::getTheta(int i)
{
	if (i < numTheta)
		return thetavalue[i];
	else
...
void BINGPP::setTheta(int i, float v)
{
	if (i < numTheta)
		thetavalue[i] = v;
	else
...
int BINGPP::getNumTheta()
{
	return numTheta;
}

void BINGPP::setNumTheta(int v)
{
	numTheta = v;
}

void BINGPP::setColorSpace(int clr)
{
	_Clr = clr;
...
int BINGPP::loadTrainedModel(string modelName) // Return -1, 0, or 1 if partial, none, or all loaded
{
	if (modelName.size() == 0)
...
void BINGPP::predictBBoxSI(Mat &img3u, ValStructVec<float, Vec4i> &valBoxes, vecI &sz, int NUM_WIN_PSZ, bool fast)
{
...
void BINGPP::predictBBoxSII(ValStructVec<float, Vec4i> &valBoxes, const vecI &sz)
{
...
void BINGPP::getBINGBoxes(Mat img3u, int numDetPerSize,
...
void BINGPP::CannyEdgeDetection(Mat img3u,Mat &edge)
{
	const int imgW = img3u.cols, imgH = img3u.rows; 
	Mat gray;
...
void BINGPP::DistanceTransform(Mat img3u, Mat edge, Mat &v, Mat &u)
{
...
void BINGPP::EdgeRecursiveBox(int rat, Mat img3u, Mat v, Mat u, ValStructVec<float, Vec4i> &valBoxesTemp)
{
	for(int i = 0; i < valBoxesTemp.size(); i++)
	{
            valBoxesTemp[i][0] = ceil(float(valBoxesTemp[i][0])/rat);
            valBoxesTemp[i][1] = ceil(float(valBoxesTemp[i][1])/rat);
            valBoxesTemp[i][2] = floor(float(valBoxesTemp[i][2])/rat);
            valBoxesTemp[i][3] = floor(float(valBoxesTemp[i][3])/rat);
	}
	...
	for (int i = 0; i < NUM_BOX; i++) {
		...
		valBoxesTemp[i][0] = max(1, box[0])*rat;
		valBoxesTemp[i][1] = max(1, box[1])*rat;
		valBoxesTemp[i][2] = min(imgW, box[2] + 2)*rat;
		valBoxesTemp[i][3] = min(imgH, box[3] + 2)*rat;
	}

}
void BINGPP::EdgesRefine(Mat &img3u, ValStructVec<float, Vec4i> &valBoxesTemp){
	int rat = 3;
	resize(img3u, img3u, Size(),1.0/rat,1.0/rat,0);

	Mat edge;
	CannyEdgeDetection(img3u, edge);

	const int imgW = img3u.cols, imgH = img3u.rows, imgS = imgH*imgW;
	Mat v(imgH, imgW, CV_32S), u(imgH, imgW, CV_32S);
	DistanceTransform(img3u, edge, v, u);

	EdgeRecursiveBox(rat, img3u, v, u, valBoxesTemp);
}

void BINGPP::SegRefine(Mat img3u,
			   ValStructVec<float, Vec4i> &valBoxesTemp,
			   ValStructVec<float, Vec4i> &valBoxes){
	Mat imgLab;
	vector<Vec4i> spBoxes;
	cvtColor(img3u, imgLab, CV_BGR2Lab);
	Segmentation(spBoxes, imgLab, 120.0f, 6);

	vecF thetas(3);
	thetas[0] = 0.1, thetas[1] = 0.3, thetas[2] = 0.6;//for best perfomance

	SegmentRecursiveBox(spBoxes, valBoxesTemp, valBoxes, thetas, 0.8, true);
}
...
void BINGPP::getObjBndBoxes(Mat &img3u, ValStructVec<float, Vec4i> &valBoxes, int numDetPerSize)
{
	CV_Assert(filtersLoaded());
	double ratioX = img3u.cols / 400.0, ratioY = img3u.rows / 360.0;
	resize(img3u, img3u, Size(400, 360));

	ValStructVec<float, Vec4i> valBoxesTemp;
	getBINGBoxes(img3u,numDetPerSize,valBoxesTemp);
	//if (valBoxesTemp.size() > 1000)
		//valBoxesTemp.resize(1000);

	EdgesRefine(img3u, valBoxesTemp);
	resize(img3u, img3u, Size(400, 360));

	SegRefine(img3u, valBoxesTemp, valBoxes);
...
void BINGPP::getObjBndBoxesEva(Mat &img3u, ValStructVec<float, Vec4i> &valBoxes, ValStructVec<float, Vec4i> &evaBoxes, vecI &sz, int numDetPerSize)
{
	CV_Assert(filtersLoaded());
...
void BINGPP::nonMaxSup(CMat &matchCost1f, ValStructVec<float, Point> &matchCost, int NSS, int maxPoint, bool fast)
{
...
void BINGPP::gradientMag(CMat &imgBGR3u, Mat &mag1u)
{
	switch (_Clr){
	case MAXBGR:
...
void BINGPP::gradientRGB(CMat &bgr3u, Mat &mag1u)
{
...
void BINGPP::gradientGray(CMat &bgr3u, Mat &mag1u)
{
	Mat g1u;
...
void BINGPP::gradientHSV(CMat &bgr3u, Mat &mag1u)
{
	Mat hsv3u;
...
void BINGPP::gradientXY(CMat &x1i, CMat &y1i, Mat &mag1u)
{
...
void BINGPP::trainBINGPP(int numDetPerSize)
{
	//CmTimer tm1("Train1"), tm2("Train 2");
        StopWatchLinux tm1,tm2;
	...
	trainStageI();
	tm1.stop();
	cout << "Learning stage I takes " << tm1.getTime() << " mseconds..." << endl;

	//* Learning stage II
	tm2.start();
	trainStageII(numDetPerSize);
	tm2.stop();
	cout << "Learning stage II takes " << tm2.getTime() << "mseconds..." << endl;
	return;
}

void BINGPP::generateTrianData()
{
	//backtrace();
...
Mat BINGPP::getFeature(CMat &img3u, const Vec4i &bb)
{
...
int BINGPP::gtBndBoxSampling(const Vec4i &bbgt, vector<Vec4i> &samples, vecI &bbR)
{
...
void BINGPP::trainStageII(int numPerSz)
{
	loadTrainedModel();
	const int NUM_TRAIN = _voc.trainNum;
...
void BINGPP::meanStdDev(CMat &data1f, Mat &mean1f, Mat &stdDev1f)
{
...
vecD BINGPP::getVector(const Mat &_t1f)
{
	Mat t1f;
...
Mat BINGPP::trainSVM(CMat &X1f, const vecI &Y, int sT, double C, double bias, double eps)
{
	// Set SVM parameters
	parameter param; {
...
Mat BINGPP::trainSVM(const vector<Mat> &pX1f, const vector<Mat> &nX1f, int sT, double C, double bias, double eps, int maxTrainNum)
{
	vecI ind(nX1f.size());
...
void BINGPP::getObjBndBoxesForTests(vector<vector<Vec4i> > &_boxes, int numDetPerSize)
{
...
#pragma omp parallel for
		...
		tm.stop();
		cout << "Average time for predicting an image (" << _clrName[_Clr] << ") is " << tm.getTime() / Num << "s" << endl;
	}
...
// Get potential bounding boxes for all test images
void BINGPP::getObjBndBoxesForTestFast(vector<vector<Vec4i> > &_boxes, int numDetPerSize)
{
	//setColorSpace(HSV);
...
#endif
#endif
	releaseSegMask(MAX_THREAD_NUM);
	
	tm.stop();
	cout << "Average time for predicting an image (" << _clrName[_Clr] << ") is " << tm.getTime() / Num << "ms" << endl;
...
void BINGPP::getObjBndBoxesForTrainsEva(vector<vector<Vec4i> > &_boxesTrains, int numDetPerSize)
{
	//trainObjectness(numDetPerSize);
	loadTrainedModel();
	Mat tmpMat;
...
#pragma omp parallel for
	...
	tm.stop();
	cout << "Average time for predicting an image (" << _clrName[_Clr] << ") is " << tm.getTime() / TrainNum << "ms" << endl;
...
void BINGPP::getRandomBoxes(vector<vector<Vec4i> > &boxesTests, int num)
{
...
void BINGPP::evaluatePerImgRecall(const vector<vector<Vec4i> > &boxesT, CStr &saveName, const int NUM_WIN)
{
...
void BINGPP::analysePerImg(vector<vector<Vec4i> > &boxesTrains, vector<vector<Vec4i> > &boxesEvas, vector<vecI> &sz)
{
...
void BINGPP::evaluateShift(vector<vector<Vec4i> > &boxesRes, vector<vector<Vec4i> > &shift)
{
#ifdef TRAIN_SET
	const int NUM = _voc.trainSet.size();
#else
...
void BINGPP::illuTestReults(const vector<vector<Vec4i> > &boxesTests)
{
...
void BINGPP::evaluatePerClassRecall(vector<vector<Vec4i> > &boxesTests, CStr &saveName, const int WIN_NUM)
{
...
void BINGPP::PrintVector(FILE *f, const vecD &v, CStr &name)
{
...
bool BINGPP::matWrite(CStr& filename, CMat& _M){
	Mat M;
	_M.copyTo(M);
...
bool BINGPP::matRead(const string& filename, Mat& _M){
...
void BINGPP::evaluatePAMI12(CStr &saveName)
{
	const int TEST_NUM = _voc.testSet.size();
...
void BINGPP::evaluateIJCV13(CStr &saveName)
{
	const int TEST_NUM = _voc.testSet.size();
	vector<vector<Vec4i> > boxesTests(TEST_NUM);
...
void BINGPP::evaluateECCV14(CStr &saveName) {
	const int TEST_NUM = _voc.testSet.size();
	vector<vector<Vec4i> > boxesTests(TEST_NUM);
...
Mat BINGPP::aFilter(float delta, int sz)
{
...
void BINGPP::illutrateLoG()
{
	for (float delta = 0.5f; delta < 1.1f; delta+=0.1f){
		Mat f = BINGPP::aFilter(delta, 8);
...
int BINGPP::minIdx(Mat &input, int x1, int y1, int x2, int y2) {
	int min = 1000000;
	for (int i = y1; i <= y2; i++) {
...
int BINGPP::maxIdx(Mat &input, int x1, int y1, int x2, int y2) {
	int max = -1;
	for (int i = y1; i <= y2; i++) {
		int *data = input.ptr<int>(i);
		for (int j = x1; j <= x2; j++) {
