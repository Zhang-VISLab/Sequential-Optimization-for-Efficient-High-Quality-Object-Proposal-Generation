...
struct ValStructVec
{
	...
	inline void clear() {sz = 0; vector<ST>().swap(structVals); vector < pair<VT, int>>().swap(valIdxes);}
	...
	inline void resize(int resSz){ sz = resSz; structVals.resize(resSz); valIdxes.resize(resSz); }
	inline void pushBack(const VT& val, const ST& structVal) {valIdxes.push_back(make_pair(val, sz)); structVals.push_back(structVal); sz++;}
        inline void erase(int id){  valIdxes.erase(valIdxes.begin()+id); sz = valIdxes.size(); }
